...
int main(int argc, char** argv) {
  ...
  auto sample_rate_result = meta_module->execute("sample_rate", empty_inputs);
  ...
  if (!sample_rate_result.ok() || !window_stride_result.ok() ||
      !encoder_subsampling_factor_result.ok()) {
    ET_LOG(
        Error,
        "Failed to query timestamp metadata (sample_rate, window_stride, encoder_subsampling_factor).");
    return 1;
  }
